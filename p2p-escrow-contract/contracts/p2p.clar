...
(define-public (register-merchant (full-name (string-utf8 100)) (supported-banks (list 2 (string-utf8 50))))
  (begin
...
(define-public (request-exchange (amount-stx uint) (account-number (string-ascii 20)) (account-name (string-ascii 50)) (bank-name (string-ascii 50)))
  (let ((request-id (+ u1 (var-get request-id-count))))
    (map-set exchange-requests { request-id: request-id } { requester: tx-sender, amount-stx: amount-stx, bank-account: { number: account-number, name: account-name, bank-name: bank-name } })
